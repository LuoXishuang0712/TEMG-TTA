...
motifs_file_prefix = args.motifs_file_prefix or '1800'
def get_dataset(dataset: str, motif_real_prefix: str | None = None):
    if motifs_enabled and dataset in motifs_datasets:
        assert motif_real_prefix is not None
        print(f"Enable {motifs_file_prefix}_{motif_real_prefix} motifs for dataset {dataset}")
        return MotifsDataset(dataset, motifs_file_rule=lambda x: 'exam_motifs/' + motifs_file_prefix + '/' + x.split('.')[0] + '.' + motif_real_prefix + '.npy')
...
try:
    for model in models:
        ...
        for dataset_name in datasets:
            ...
            for motifs_real_prefix in ['agg_1800_ts', 'agg_3600_ts', 'agg_7200_ts', 'w_100_agg_1800_ts', 'w_100_agg_3600_ts', 'w_100_agg_7200_ts', 'w_100_ts', 'w_200_agg_1800_ts', 'w_200_agg_3600_ts', 'w_200_agg_7200_ts', 'w_200_ts', 'w_50_agg_1800_ts', 'w_50_agg_3600_ts', 'w_50_agg_7200_ts', 'w_50_ts']:
                data = get_dataset(og_dataset_name, motifs_real_prefix)
                model_config = {'model': model, 'lr': 0.01, 'drop_rate': 0}
                if og_dataset_name == 'tsocial':
                    model_config['h_feats'] = 16
                    # if model in ['GHRN', 'KNNGCN', 'AMNet', 'GT', 'GAT', 'GATv2', 'GATSep', 'PNA']:   # require more than 24G GPU memory
                        # continue

                dataset_name = og_dataset_name + '|' + motifs_real_prefix
