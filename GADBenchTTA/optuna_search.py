...
for model in models:
    ...
    for dataset_name in datasets:
        ...
        #         break
        #     try:
        #         model_config = sample_trial_param(trial, model, dataset_name, t)
        ...
        def run_search(trial):
            global time_cost, best_val_score
            t = trial.number
            model_config = sample_trial_param(trial, model, dataset_name, t=t)
            detector = model_detector_dict[model](train_config, model_config, data)
            train_config['seed'] = seed_list[t]
            st = time.time()
            print("model_config: ", model_config)
            ...
            print("Current Val Best:{:.4f}; Test AUC:{:.4f}, PRC:{:.4f}, RECK:{:.4f}".format(
                detector.best_score, test_score['AUROC'], test_score['AUPRC'], test_score['RecK']))
            trial.set_user_attr("TEST_RESULT", test_score)
            return test_score['AUPRC']
        
        os.makedirs(f"./optuna_db/{model}/", exist_ok=True)
        study = optuna.create_study(direction='maximize', study_name=f'{model} - {dataset_name} optimize study', storage=f"sqlite:///./optuna_db/{model}/{model}-{dataset_name}.db")
        study.optimize(run_search, n_trials=args.trials)
        best_model_config = study.best_trial.params
        best_result = study.best_trial.user_attrs['TEST_RESULT']
        
        print("best_model_config:", best_model_config)
        best_model_configs[model][dataset_name] = deepcopy(best_model_config)
        model_result[dataset_name+'-AUROC'] = best_result['AUROC']
        model_result[dataset_name+'-AUPRC'] = best_result['AUPRC']
        model_result[dataset_name+'-RecK'] = best_result['RecK']
