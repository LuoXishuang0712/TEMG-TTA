...
parser.add_argument('--tta_trials', type=int, default=10)  # TODO
...
log_prefix = args.log_prefix or 'tta'
...
def look_motif_rep(model, dataset_name):
    try:
        ...
        try:
            rep_role = model.feature_fusion.motifs_expression.learnable_role_embed.detach()
            rep_motifs = model.feature_fusion.motifs_expression.learnable_motifs_embed.detach()
        except:
...
def get_dataset(dataset: str):
    if motifs_enabled and dataset in motifs_datasets:
        ...
        return MotifsDataset(dataset, motifs_file_rule=lambda x: 'exam_motifs/3600/' + x.split('.')[0] + '.' + motifs_file_prefix + '.npy')
...
file_id = None
motif_prefix = "with_motif" if motifs_enabled else "no_motif"
try:
    for model in models:
        ...
        for dataset_name in datasets:
            ...
            with torch.no_grad():
                _, emb = best_model.get_emb(data.graph.to(train_config['device']))
                np.save(f'emb_file/{dataset_name}_{model}_train_{motif_prefix}_emb.npy', emb.detach().cpu().numpy())
            del detector, data
            run_gc()
            ...
            for tta_dataset_name in tta_datasets:
                ...
                for tta_class in tta_classes:
                    ...
                    tta_method = tta_class(best_model, tta_dataset.graph, og_feature_dim, en_sec_device=(model in ['SpaceGNN', 'ARC']))
                    try:
                        tta_method.adapt()
                        ...
                        print(f"Compare {model} and {tta_class.__name__} on {tta_dataset_name}: ", compare_model_feature(og_model_feature, current_model_feature))
                        for history_tta_class, history_model in history_model_feature.items():
                            print(f"Compare {tta_class.__name__} and {history_tta_class} on {tta_dataset_name}: ", compare_model_feature(current_model_feature, history_model))
                        history_model_feature[tta_class.__name__] = current_model_feature
                    except torch.cuda.OutOfMemoryError:
                    ...
                    for metric in ['AUROC', 'AUPRC', 'RecK', 'F1', 'ACC']:
                        model_result['TTA-'+tta_class.__name__+'-'+dataset_name+'-'+tta_dataset_name+'-'+metric+' mean'] = tta_test_score[metric]
                        model_result['TTA-'+tta_class.__name__+'-'+dataset_name+'-'+tta_dataset_name+'-'+metric+' std'] = 0.0
                    with torch.no_grad():
                        model_device = next(tta_method.get_trained_model().parameters()).device
                        _, emb = tta_method.get_trained_model().get_emb(tta_dataset.graph.to(model_device))
                        np.save(f'emb_file/{dataset_name}_{model}_{tta_class.__name__}_{tta_dataset_name}_{motif_prefix}_emb.npy', emb.detach().cpu().numpy())
                    del tta_method
                    run_gc()
                    ...
                    for group in tta_dataset.iter_group():
                        ...
                        for metric in ['AUROC', 'AUPRC', 'RecK', 'F1', 'ACC']:
                            model_result['TTA-'+tta_class.__name__+'-'+dataset_name+'-'+tta_dataset_name+'@'+str(group)+'-'+metric+' mean'] = tta_test_score[metric]
                            model_result['TTA-'+tta_class.__name__+'-'+dataset_name+'-'+tta_dataset_name+'@'+str(group)+'-'+metric+' std'] = 0.0
                        del tta_method
                        run_gc()
                del tta_dataset
